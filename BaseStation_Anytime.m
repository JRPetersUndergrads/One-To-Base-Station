    ...
    methods
        ...
        function obj = OneToBaseUpdate(obj,Agent,width,height,transparancy,BaseFig,dt)%,newtime,dt)
            ...
            if obj.AgentTimers(Agent) <=0 %only update Agent if it doesn't have timer on it
                ...
                numChecks = 10+floor(rand*91); %Number of nodes the code is allowed to process.
                nodesChecked = 0; comFail = 0;
                ...
                k = thisCoveringNotCenter;
                currentChecked = obj.AgentCheckedRegions(Agent);
                for i = currentChecked+1:length(thisCoveringNotCenter);
                    %%[currentChecked+1:length(thisCoveringNotCenter),1:currentChecked] %Loop through all nodes?
                    %clc
                    disp([num2str(i),' out of ' ,num2str(length(thisCoveringNotCenter))]);
                    [compareCoverings compareCost] = obj.FindRegions(Agent,k(i));
                    %if this k gives us better cost, use k
                    if compareCost<lowCost
                        NewCoverings = compareCoverings;
                        lowCost = compareCost;
                        c = k(i);
                        disp(['Update with agent found success, resetting all agents currentChecked node.'])
                        obj.AgentCheckedRegions = zeros(size(obj.AgentCheckedRegions));
                    end
                    nodesChecked = nodesChecked + 1;
                    if nodesChecked >= numChecks;
                        disp(['Communication with agent terminated.'])
                        disp(['Occured after ', num2str(nodesChecked), ' nodes, at node# ', num2str(i)])
                        comFail = 1;
                        obj.AgentCheckedRegions(Agent) = i;
                        break;
                    end
                    if i == length(thisCoveringNotCenter);
                        disp(['Communication with agent finished.'])
                        disp(['Occured after ', num2str(nodesChecked), ' nodes, at node# ', num2str(i)])
                        comFail = 0;
                    end
                end
                ...
                if comFail == 0
                    obj.AgentCheckedRegions(Agent) = length(thisCoveringNotCenter);
                end
                ...
                [Centroid, IsCentroidal] = obj.FindCentroid(Agent,c);
                if IsCentroidal
                    disp('Current Agent''s Region is CENTROIDAL!')
                else
                    disp('Current Agent''s Region is not Centroidal,')
                    disp(['Actual Centroid at: ', num2str(Centroid)])
                end
            end
        ...
        function [Centroid, IsCentroidal] = FindCentroid(obj,Agent,c)
            CurrentCoverings = obj.Coverings;
            CoveringCoordinates = obj.Map.xy(CurrentCoverings{Agent},:);
            CoveringIndicies = obj.Map.PointsIndices(CurrentCoverings{Agent});
            CurrentDensity = obj.Map.Densities(CurrentCoverings{Agent});

            CurrentCenter = obj.Map.xy(obj.Centers(Agent),:);
            
            Xnum = 0; Xden = 0;
            Ynum = 0; Yden = 0;
            for i = 1:length(CoveringCoordinates)
                x = CoveringCoordinates(i,1);
                y = CoveringCoordinates(i,2);
                density = CurrentDensity(i);
                Xnum = Xnum + (x*density);
                Ynum = Ynum + (y*density);
                Xden = Xden + density;
                Yden = Yden + density;
            ...
            CentroidNode = 0;
            CentroidMindist = inf;
            for i = 1:length(CoveringCoordinates) %We're only looking at points INSIDE the covering to be closest...?
                x = CoveringCoordinates(i,1);
                y = CoveringCoordinates(i,2);
                if CentroidMindist > sqrt((Xcent-x)^2 + (Ycent-y)^2)
                    Centroid = CoveringIndicies(i);
                    CentroidMindist = sqrt((Xcent-x)^2 + (Ycent-y)^2);
                end
            end
            if CentroidMindist >= sqrt((Xcent-CurrentCenter(1))^2 + (Ycent-CurrentCenter(2))^2)
                Centroid = c;
                IsCentroidal = true;
            else
                IsCentroidal = false;
