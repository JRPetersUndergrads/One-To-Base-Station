...
timeUncovered = zeros(1,NGridx*NGridy);
...
for t = 1:dt:time
    ...
    totalCovered = []; %list of points that are actively covered by agents
    for i = 1:NRegions
        thisCovered = Agents{i}.Map.PointsIndices;%points in this agents active coverings
        thisCovered = thisCovered(~ismember(thisCovered,totalCovered)); %remove points already counted
        totalCovered = [totalCovered thisCovered];
        notCovered = ~ismember(CompleteMap.PointsIndices,totalCovered);
    end
    timeUncovered = timeUncovered +dt.*notCovered;
end
    timeUncoveredDisplay = zeros(NGridx,NGridy);
    for i = 1:length(timeUncovered)
        [I,J] = ind2sub([NGridx,NGridy],i);
        timeUncoveredDisplay(I,J) = timeUncovered(i);
        figure(3)
        hold on
        mesh(timeUncoveredDisplay);
        hold off
    end